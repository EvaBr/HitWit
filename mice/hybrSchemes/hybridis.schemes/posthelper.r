postprocess <- function ( tab, tissue, fajl, flagy ) {
  tab[,2:5][tab[,2:5]==""] <- "NA"
  ...
  write.csv ( tab[,2:6], file = paste("../Awesome.Reading.Tool/results/", fajl, ".csv", sep=""), na = "NA", row.names = tab[,1] )
...
doReading <- function ( tab, tissue ) {
  ...
  #print(tab)
  return ( list(Table=tab, Flag=flag) )
}
